...
#[tauri::command]
pub async fn claim_logo_reward(
    account_id: String,
    session_token: String,
) -> Result<RewardResult, String> {
    claim(&account_id, &session_token)
        .await
...
async fn claim(account_id: &str, session_token: &str) -> AppResult<RewardResult> {
    ...
    if config.base_url.contains("CHANGE_ME") {
        return Err(AppError::Message(
            "Backend-URL für den Secret-Reward ist noch nicht konfiguriert.".into(),
        ));
    }

    let base_value = format!("{}/", config.base_url.trim_end_matches('/'));
    let base =
        Url::parse(&base_value).map_err(|_| AppError::Message("Ungültige Backend-URL".into()))?;
    if base.scheme() != "https" && !(config.allow_insecure_http && base.scheme() == "http") {
        return Err(AppError::Message(
            "Backend-Verbindung benötigt HTTPS.".into(),
        ));
    }
    ...
    if token.is_empty() {
        return Err(AppError::Message(
            "Die Backend-Session ist ungültig. Bitte den Account erneut auswählen.".into(),
        ));
    }
    ...
    let response = client
        .post(
            base.join("rewards/logo-secret")
                .map_err(|_| AppError::Message("Ungültige Reward-URL".into()))?,
        )
