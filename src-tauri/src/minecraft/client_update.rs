use crate::{app::paths::GamePaths, error::{AppError, AppResult}, logging};
...
pub async fn sync_latest(game: &GamePaths) -> AppResult<ClientUpdateResult> {
    ...
    if !config.enabled || config.url.contains("CHANGE_ME") {
        return Ok(ClientUpdateResult { updated: false, hash: file_sha256_optional(&target)?.unwrap_or_default() });
    }

    let url = Url::parse(&config.url).map_err(|_| AppError::Message("Ungültige Client-Update-URL".into()))?;
    if url.scheme() != "https" && !(config.allow_insecure_http && url.scheme() == "http") {
        return Err(AppError::Message("Client-Updates benötigen HTTPS. HTTP muss ausdrücklich freigeschaltet werden.".into()));
    }
    ...
    let stored_etag = fs::read_to_string(&etag_path).ok().map(|value| value.trim().to_string());
    if target.exists() {
        if let Ok(response) = client.head(url.clone()).send().await {
            if response.status().is_success() {
                let remote_etag = response.headers().get(reqwest::header::ETAG).and_then(|value| value.to_str().ok()).map(str::to_string);
                if remote_etag.is_some() && remote_etag == stored_etag {
                    return Ok(ClientUpdateResult { updated: false, hash: file_sha256(&target)? });
                }
            }
        }
    }
    ...
    if bytes.len() < 32_768 || !bytes.starts_with(b"PK") {
        return Err(AppError::Message("Die heruntergeladene Client-Datei ist keine gültige JAR.".into()));
    }

    let remote_hash = bytes_sha256(&bytes);
    if target.exists() && file_sha256(&target)? == remote_hash {
        if let Some(etag) = remote_etag { write_atomic(&etag_path, etag.as_bytes()).await?; }
        return Ok(ClientUpdateResult { updated: false, hash: remote_hash });
    }
    ...
    if file_sha256(&temp)? != remote_hash {
        ...
        return Err(AppError::Message("SHA-256-Prüfung der Client-JAR ist fehlgeschlagen.".into()));
    }
    if backup.exists() { fs::remove_file(&backup)?; }
    if target.exists() { fs::rename(&target, &backup)?; }
    ...
    Ok(ClientUpdateResult { updated: true, hash: remote_hash })
}
...
fn bytes_sha256(bytes: &[u8]) -> String { hex::encode(Sha256::digest(bytes)) }
fn file_sha256(path: &Path) -> AppResult<String> { Ok(hex::encode(Sha256::digest(fs::read(path)?))) }
fn file_sha256_optional(path: &PathBuf) -> AppResult<Option<String>> { if path.exists() { Ok(Some(file_sha256(path)?)) } else { Ok(None) } }
